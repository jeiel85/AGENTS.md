...
> 아래 §30–§41은 **GitHub 협업 및 공개 저장소 운영 방침**입니다. 에이전틱 코딩 환경에서 GitHub 협업 품질, 공개 저장소 완성도, 릴리스 신뢰성, 지표 무결성을 유지하기 위한 규칙이며, 위의 공통 규칙(§1–§29)과 함께 적용합니다.
>
...
이 저장소의 작업 품질 기준은 **실행 모델의 등급과 무관하게 동일**합니다. 최상위 모델에서 하위·경량 모델(예: 최상위 티어 → Opus, Sonnet, Haiku 등)로 전환되더라도, 판단력 차이는 아래의 절차적 엄격함으로 보정합니다. 이 섹션은 상위 모델이 자연스럽게 수행하는 작업 습관을 모든 모델의 명시적 의무로 전환한 것입니다.
...
- 3단계 이상이 되는 작업은 체크리스트(todo)로 관리하고, 단계가 끝날 때마다 상태를 갱신합니다.
...
- 요구사항 목록과 구현 결과를 1:1로 대조하여 누락을 확인합니다.
- 하위 모델일수록 이 단계를 생략하지 않습니다. 자기 검증 없이 선언한 완료는 완료로 인정하지 않습니다.

### 43.6 실패 처리 (No Silent Give-Up)

- 에러가 발생하면 로그 전체를 읽고 원인을 좁힌 뒤 수정하여 재시도합니다.
- 같은 접근이 3회 실패하면 접근 방식을 바꿉니다. 그래도 해결되지 않으면 시도 내역, 실패 로그, 남은 가설과 함께 `BLOCKED`로 보고합니다. (§40 Completion Criteria 적용)
- "아마 해결되었을 것" 상태로 작업을 종료하지 않습니다.

### 43.7 모델 사용 전략 (Model Allocation)

- 가능하면 계획 수립, 설계 판단, 최종 리뷰는 사용 가능한 가장 강한 모델 세션에서 수행합니다.
- 기계적 구현, 반복 수정, 문서 갱신은 경량 모델로 수행하되, 이 하네스를 더 엄격하게 적용합니다.
