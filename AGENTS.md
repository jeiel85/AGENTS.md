...
사용자가 코드·기능 변경을 요청하는 "goal" 작업은 기본적으로 아래 3단계로 진행합니다(사소한 한 줄 수정, 순수 질의응답, 즉시 자명한 기계적 편집은 예외). 단계별로 모델을 바꿀 수 있는 환경에서는 각 단계에 지정 등급을 사용하고, 하위 에이전트에 모델을 지정할 수 있는 오케스트레이터 환경(Workflow, ultracode 등)에서는 한 세션 안에서 서브에이전트 위임으로 대체합니다. 이 프로토콜은 §43.1–§43.8을 대체하지 않고 그 위에서 실행 구조를 정합니다.

- **1단계 — 분석·설계 (강한 모델: Opus 이상 권장)**: 수정 전 코드베이스를 실제로 읽어 데이터 흐름, 호출부, 저장 구조를 파악하고, "바로 수정 가능한 수준"의 구현 지점(파일, 함수, 앵커, 데이터 계약)을 특정합니다(§43.2 근거 우선). 이름과 실제 데이터가 어긋나는 등 확인이 필요한 지점은 근거와 함께 먼저 사용자에게 확인합니다(§3). 결과를 계약(변경 파일, 함수 시그니처, 데이터 형태, 시드 값)으로 고정해 이후 단계의 기준으로 삼습니다.
- **2단계 — 구현 (Sonnet 등 경량 모델 + ultracode/Workflow 멀티에이전트)**: 1단계에서 고정한 계약과 정확한 앵커·코드를 각 에이전트에 전달하고, 서로 독립적인 파일·영역은 병렬로 적용합니다. 각 에이전트는 자기 영역만 편집하고 가능한 즉시 검증합니다(§43.3). 경량 모델이라도 §43.1–§43.7을 생략하지 않습니다.
- **3단계 — 감사 (강한 모델: Opus 이상 권장)**: 구현 결과를 신뢰하지 않고 전체 diff를 다시 읽어 1단계 요구사항·계약과 1:1로 대조하고(§43.5, §16), 실행 가능한 검증(구문 체크, 핵심 로직 실측, 브라우저·런타임 확인 등)으로 실제 동작을 확인한 뒤에만 완료를 보고합니다(§40, §27). 계약과 어긋난 변경, 범위를 벗어난 변경(§43.7)은 되돌리거나 바로잡습니다.
- 이 프로토콜을 적용한 작업의 최종 보고에는 각 단계에서 사용한 모델·에이전트 구성과 3단계 감사의 실제 검증 근거를 함께 명시합니다(§27 Final Report Format).
