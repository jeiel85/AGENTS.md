...
자동 진행은 작업을 중간 확인 없이 끝까지 수행한다는 의미이며, 변경을 어떤 방식으로 반영할지(직접 커밋 또는 Pull Request)는 §30 Pull Request First를 따릅니다. GitHub로 추적·공유되거나 공개된 저장소에서는 기능, 버그 수정, 리팩터링, 빌드 설정, 의존성 변경을 기본적으로 작업 브랜치와 Pull Request로 반영합니다.
...
단, GitHub로 협업하거나 공개된 저장소에서는 §30 Pull Request First에 따라 기능, 버그 수정, 리팩터링, 빌드 설정, 의존성 변경을 작업 브랜치와 Pull Request로 반영하며, `main` 직접 커밋은 §30에 정의된 예외(단순 오탈자 수정, 긴급 CI 복구, 저장소 초기 구성, 사용자가 명시적으로 직접 반영을 요청한 경우)에만 허용합니다.
...
공개·공유 저장소에서는 §34 Pull Request Quality의 상세 본문 형식(Summary / Motivation / Changes / Validation / Risks / Screenshots / Related Issues)을 우선 사용합니다. 브랜치 이름은 §30 Branch Naming 규칙을 따릅니다.
...
## 30. Pull Request First

- 사용자 기능, 버그 수정, 리팩터링, 빌드 설정, 의존성 변경은 원칙적으로 작업 브랜치와 Pull Request를 통해 반영합니다.
- 하나의 Pull Request는 하나의 명확하고 검증 가능한 목적을 가져야 합니다.
- 구현 코드, 관련 테스트, 필요한 문서는 동일한 변경 단위로 관리합니다.
- Pull Request 수를 늘리기 위해 하나의 작업을 인위적으로 여러 개로 분리하지 않습니다.
- 다음 경우에만 `main` 브랜치 직접 커밋을 허용합니다.
  - 단순 오탈자 수정
  - 긴급 CI 복구
  - 저장소 초기 구성
  - 사용자가 명시적으로 직접 반영을 요청한 경우
- 직접 커밋이 허용되더라도 빌드, 테스트, 정적 분석, 문서 검증 절차를 생략하지 않습니다.

이 규칙은 §2 Automation First, §21 커밋·푸시·배포, §22 브랜치/PR 규칙과 함께 적용합니다. 자동 진행(중간 확인 없이 끝까지 수행)과 Pull Request First(변경을 어떤 방식으로 반영할지)는 서로 충돌하지 않으며, GitHub로 추적·공유되거나 공개된 저장소에서는 작업 브랜치와 PR로 반영하는 것을 기본으로 합니다.
