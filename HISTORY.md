...
- 변경 파일:
  ...
  - HISTORY.md: 작업 이력 기록
- 배경: 사용자 지시로 goal 작업의 표준 실행 구조를 명문화. 실제 `kunsarang-baptist-church` '주일학교 설교 스케쥴' 기능을 이 3단계(Opus 분석 → Sonnet ultracode 구현 → Opus 감사)로 수행·검증한 뒤 규칙화. 동일 내용을 전역 지침(`~/.claude/CLAUDE.md`)에도 반영
